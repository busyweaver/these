...
def coef(r,N,i):
    x = binomial(N-1-r, i-1)*binomial(r,i)*math.pow(2,r-i)
    return x
...
def trouvernoeuds(N,k,ab,i,ab2,noeuds):
    l = composition(N-k,i)
    comp=l[int(math.ceil(ab2/float(binomial(k,i))))-1]
...
def const_arbre(N,i,k,comp,noeuds):
    ...
    #je crois que cest la partie a commenter
...
def somnk(n,k):
    ...
    for i in range(1,n-k+1):
        tab.append(pow(2,k-i)*binomial(n-1-k,i-1)*binomial(k,i))
    som_nk[(n,k)]=tab 

# for z in range(1,N+1):
#     CON.append([])
#     for b in range(0,int(z*(z-1)*0.5)+1):
#         CON[z].append(calcul_CO(z,kf,b))
